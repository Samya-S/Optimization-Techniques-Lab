...
bool check_is_basic(int col){
    for (auto e : basis) if (e == col) return true;
    return false;
}
...
void print_table(int entering_col){
    ...
    for (int i = 0; i<table.size(); i++){
        ...
        if (i==0) cout << "-";
        else if (table[i][table[0].size() - 1] > 0) cout << table[i][table[0].size() - 2] / table[i][table[0].size() - 1];
        else cout << "-";
...
int main(){
    ...
    if (num_unrestricted) cout << "Enter the indices of the unrestricted variables : ";
    ...
    for (int i = 0; i<numvars; i++){
        vartype.push_back(0);
        if (unrestricted[i]) vartype.push_back(-1);
    }
    ...
    for (int i = 0; i<numvars; i++){
        ...
        if (unrestricted[i]) func_coeff.push_back(coeff * (maximise==true ? -1 : 1));
    ...
    for (int i = 0; i<vartype.size(); i++){
        ...
        if (e){
            // vartype 1 for slack
            if (e==1) slack_vars.push_back(i);
            // vartype 2 for surplus
            else if (e==2) surplus_vars.push_back(i);
            // vartype 3 for artificial
            else artificial_vars.push_back(i);
        }
    }

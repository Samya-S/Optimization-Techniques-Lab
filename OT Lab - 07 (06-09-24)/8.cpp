...
public:
    Simplex(int variables, int constraints) : n(variables), m(constraints), A(constraints, vector<double>(variables + constraints + constraints + variables)), c(variables + constraints + constraints + variables),  // c includes slack and artificial variables
                                              b(constraints),
                                              CB(constraints),
                                              B(constraints),
                                              neg(variables),
    ...
    void readInput() {
        ...
        if (cnt == 0) {
            ...
            cout << "big m method is used here " << endl;
        }
        ...
        vector<int> tmp_extra(n, 0);
        int tmp_ex = extravar;
        for (int i = 0; i < n; i++) {
            if (isunrestriced[i]) {
                tmp_extra[i] = n + tmp_ex;
                tmp_ex++;
            }
        }
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                if (isunrestriced[j] == 1) {
                    A[i][tmp_extra[j]] = -1.0 * A[i][j];
                    c[tmp_extra[j]] = -1.0 * c[j];
                    neg[j] = tmp_extra[j];
                }
            }
        }
